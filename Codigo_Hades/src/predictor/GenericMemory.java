...
public class  GenericMemory 
  ...
  public void initializeWithRandomValues() {
    if (data == null) createMemory();
    for( int i=0; i < n_words; i++ ) {
      //data[i] = Double.doubleToLongBits(Math.random()) & 0x7fffffffffffffffL;
  ...
  /**
   * initialize a GenericMemory from a String. Currently, the String should
   * contain the following things: the GenericMemory versionId, and the
   * Java resource-name of the ROM/RAM datafile to parse, e.g.
    <pre>
  ...
  ////////////////////////////////////////////////////////////////////////
  //
  // interface hades.models.Memory stuff
  //
  ////////////////////////////////////////////////////////////////////////
  ...
  public void setDataAt( int address, long value ) {
    ...
    if (value != -1) { // mask out out-of-range bits, but allow UNDEFINED
    ...
    // repaints should be requested by subclasses, not by us:
    //
    //if (MEF != null && MEF.isShowing()) {
    //   MEF.repaint( 200 );
    //}
  }
  ...
  public void setBitsPerWord( int n_bits ) throws Exception {
    if (n_bits == this.n_bits) return; // same width
    else {
      this.n_bits = n_bits;
      createMemory();
    }
  ...
  void addMemoryListener( MemoryListener ML ) {
    ...
    for( int i=0; i < _listenerCount; i++ ) {    // copy old listeners
      tmp[i] = _listenerArray[i];
    }
    tmp[ _listenerCount ] = ML;                  // add the new one
  ...
  void removeMemoryListener( MemoryListener ML ) {
    ...
    // do we have any listeners at all?
    //
    if (_listenerCount == 0) {
    ...
    if (_listenerCount == 1) { // need to remove the last listener
      _listenerArray = null;
      _listenerCount = 0;
      return;
    }
   ...
   * If the file is not found or contains illegal data, the RAM is
   * re-initialized to the all-invalid state.
   * <p>
   * Each line of the RAM data file should contain the address and
   * the corresponding data in hex format, seperated by a colon,
   * e.g. '0001:D300000000FF' or '03FF:FEDCBA9876543210'.
   * <p>
   * The alternative DigLOG compact format, e.g.
   * '0000:000102030405060708090A0B0C0D0E0F'
   * is NOT accepted, because this is useful only for byte-oriented memories.
   *  <p>
   * Input file lines starting with a '#' or ';' character are interpreted
   * as comment lines and are ignored.
   * <p>
   * The RAM model will issue a warning message, if the same location
  ...
  public boolean parse( BufferedReader br ) throws Exception {
    ...
    while( (line = lnr.readLine()) != null) {
      ...
      try {
        ...
        if (st.countTokens() < 2) continue;    // not enough data
  ...
  protected void parseLabelLine( String line ) throws Exception {
    if (_decoder == null) return; // no use without InstructionDecoder...
  ...
  protected String getHexString( long value, int n_chars ) {
    ...
    if (value < 0) { // HACK: values < 0 mean XXX
  ...
  /**
   * write the memory data in a trivial addr:data format, one
   * address per line. Both address and data are hex-formatted and
   * separated by a colon (and optionally, some blanks).
   */
  ...
  ////////////////////////////////////////////////////////////////////////
  //
  // configuration GUI and callbacks
  //
  ////////////////////////////////////////////////////////////////////////
  ...
  ////////////////////////////////////////////////////////////////////////
  //
  // simulation stuff: elaborate, evaluate, notify listeners, getToolTip
  //
  ////////////////////////////////////////////////////////////////////////
   ...
   * We write the memory class, memory size, memory name,
   * current output value, and last read and write addresses.
   */
